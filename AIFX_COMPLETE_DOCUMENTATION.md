# AIFX v2 完整系統文件
# AI-Powered Forex Trading Advisory System

---

## 目錄

1. [專案概述](#1-專案概述)
2. [系統架構](#2-系統架構)
3. [技術棧](#3-技術棧)
4. [微服務架構](#4-微服務架構)
5. [Backend API 服務](#5-backend-api-服務)
6. [ML Engine 機器學習引擎](#6-ml-engine-機器學習引擎)
7. [Discord Bot](#7-discord-bot)
8. [LINE Bot](#8-line-bot)
9. [資料庫設計](#9-資料庫設計)
10. [訂閱通知系統](#10-訂閱通知系統)
11. [交易週期設計](#11-交易週期設計)
12. [API 端點完整列表](#12-api-端點完整列表)
13. [認證與安全機制](#13-認證與安全機制)
14. [數據流程圖](#14-數據流程圖)
15. [部署指南](#15-部署指南)
16. [環境變數完整列表](#16-環境變數完整列表)
17. [開發歷程](#17-開發歷程)
18. [常見問題](#18-常見問題)

---

## 1. 專案概述

### 1.1 專案簡介

AIFX 是一個基於人工智慧的外匯交易顧問系統，旨在協助交易者做出更明智的投資決策。系統核心採用 LSTM（長短期記憶網絡）深度學習模型，分析包含開盤價、收盤價、最高價、最低價、成交量等 12 項技術特徵的歷史數據，預測未來價格走勢。同時整合 FinBERT 金融領域自然語言處理模型與 VADER 情緒分析器，即時爬取並分析財經新聞、央行政策聲明等市場資訊，量化市場情緒指標。系統將價格預測與情緒分析結果進行加權融合，依據不同交易週期（日內交易 15 分鐘、周內交易 1 小時、月內交易 1 天、季內交易 1 週）動態調整技術面與情緒面的權重比例，最終生成買入（BUY）、賣出（SELL）或持有（HOLD）的交易訊號，並附帶信心指數供用戶參考。用戶可透過 Discord Bot 或 LINE Bot 訂閱感興趣的貨幣對，當訊號發生變化時系統會即時推播通知。

### 1.2 核心功能

| 功能 | 說明 |
|------|------|
| **AI 價格預測** | 使用 LSTM 深度學習模型分析歷史 OHLCV 數據，預測價格走勢 |
| **情緒分析** | 整合 FinBERT + VADER 分析財經新聞與央行政策立場 |
| **多週期訊號** | 支援日內(15min)、周內(1h)、月內(1d)、季內(1w) 四種交易週期 |
| **即時通知** | 透過 Discord Bot 和 LINE Bot 推送訂閱的訊號變化 |
| **訂閱管理** | 用戶可自由訂閱/取消訂閱特定貨幣對的信號通知 |
| **回測系統** | 使用歷史數據驗證交易策略的績效 |
| **數據收集** | 自動收集市場數據，混合模式節省 API 配額 |
| **管理後台** | Python 桌面應用程式監控系統狀態 |

### 1.3 支援貨幣對

- **EUR/USD** (歐元/美元)
- **USD/JPY** (美元/日圓)
- **GBP/USD** (英鎊/美元)

### 1.4 目錄結構

```
AIFX_v2/
├── backend/                 # Node.js API 服務 (Port 3000)
│   ├── src/
│   │   ├── app.js          # Express 應用程式
│   │   ├── server.js       # 伺服器入口
│   │   ├── config/         # 配置檔案
│   │   ├── controllers/    # API 控制器
│   │   ├── services/       # 業務邏輯服務
│   │   ├── models/         # Sequelize ORM 模型
│   │   ├── routes/         # API 路由定義
│   │   ├── middleware/     # Express 中間件
│   │   └── utils/          # 工具函式
│   └── package.json
│
├── ml_engine/               # Python ML 引擎 (Port 8000)
│   ├── api/                # FastAPI 服務
│   │   ├── ml_server.py    # 主程式入口
│   │   ├── reversal_api.py # 反轉預測 API
│   │   ├── model_manager.py# 模型管理器
│   │   └── ab_testing.py   # A/B 測試框架
│   ├── services/           # 核心服務
│   │   └── sentiment_analyzer.py  # 情緒分析服務
│   ├── models/             # ML 模型定義
│   │   ├── price_predictor.py     # LSTM 價格預測器
│   │   └── dual_mode_reversal_predictor.py
│   ├── data_processing/    # 數據處理
│   │   ├── preprocessor.py        # 數據預處理器
│   │   └── twelvedata_fetcher.py  # Twelve Data API
│   ├── backtest/           # 回測系統
│   ├── training/           # 模型訓練腳本
│   ├── scripts/            # 執行腳本
│   ├── saved_models/       # 訓練好的模型權重
│   ├── config.yaml         # 配置檔
│   └── requirements.txt    # Python 依賴
│
├── discord_bot/             # Discord Bot
│   ├── bot.js              # 主程式入口
│   ├── deploy-commands.js  # 部署斜線命令
│   ├── commands/           # 斜線命令
│   │   ├── signal.js       # 查詢交易訊號
│   │   ├── subscribe.js    # 訂閱通知
│   │   ├── unsubscribe.js  # 取消訂閱
│   │   ├── subscriptions.js# 查看訂閱列表
│   │   ├── position.js     # 持倉建議
│   │   ├── preferences.js  # 交易偏好設定
│   │   ├── trading-guide.js# 交易指南
│   │   └── ping.js         # 連線測試
│   ├── services/           # 服務層
│   │   └── backendClient.js# Backend API 客戶端
│   └── utils/              # 工具函式
│
├── line_bot/                # LINE Bot (Port 3001)
│   ├── bot.js              # Express webhook 伺服器
│   ├── handlers/           # 訊息處理器
│   │   └── messageHandler.js
│   ├── services/           # 服務層
│   │   ├── backendClient.js   # Backend API 客戶端
│   │   └── messageBuilder.js  # Flex Message 建構器
│   └── utils/              # 工具函式
│
├── admin_app/               # Python 管理後台 (Tkinter)
│   └── aifx_admin_v2.py    # 桌面應用程式
│
├── frontend/                # React 前端 (開發中)
│   ├── src/
│   │   ├── components/     # React 組件
│   │   ├── pages/          # 頁面
│   │   ├── services/       # API 服務
│   │   └── App.jsx         # 主程式
│   └── vite.config.js
│
├── database/                # 資料庫相關
│   ├── migrations/         # Sequelize 遷移檔
│   └── seeders/            # 種子資料
│
├── docs/                    # API 文件
├── scripts/                 # 工具腳本
├── tests/                   # 測試檔案
├── logs/                    # 日誌檔案
│
├── start-all-services.sh    # 一鍵啟動所有服務
├── stop-all-services.sh     # 一鍵停止所有服務
├── check_services.sh        # 檢查服務狀態
│
├── CLAUDE.md                # Claude Code 開發規範
├── DEVELOPMENT_HISTORY.md   # 開發歷史記錄
├── README.md                # 專案說明
├── BOTS.md                  # Bot 服務說明
├── ML_ENGINE.md             # ML 引擎說明
└── SERVICES.md              # 服務說明
```

---

## 2. 系統架構

### 2.1 整體架構圖

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                              用戶端 (Client)                                  │
├───────────────────────────────┬─────────────────────────────────────────────┤
│        Discord App            │              LINE App                        │
│    (iOS/Android/Desktop)      │         (iOS/Android)                       │
└───────────────┬───────────────┴──────────────────┬──────────────────────────┘
                │                                   │
                │ Discord Gateway                   │ HTTPS Webhook
                │ (WebSocket)                       │
                ▼                                   ▼
┌───────────────────────────────┐     ┌───────────────────────────────────────┐
│       Discord Bot             │     │           LINE Bot                     │
│       (Node.js)               │     │     (Node.js + Express)               │
│                               │     │         Port: 3001                     │
│  • Slash Commands             │     │  • Text Message Handler               │
│  • Embed Messages             │     │  • Flex Message Builder               │
│  • Redis Pub/Sub              │     │  • Redis Pub/Sub                      │
└───────────────┬───────────────┘     └──────────────────┬────────────────────┘
                │                                         │
                │ REST API (X-API-Key)                   │ REST API (Bearer Token)
                │                                         │
                └────────────────────┬────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                          Backend API Server                                  │
│                      (Node.js + Express, Port: 3000)                        │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│   ┌──────────────┐   ┌──────────────┐   ┌──────────────┐   ┌────────────┐  │
│   │   Routes     │──▶│ Controllers  │──▶│   Services   │──▶│   Models   │  │
│   │   路由層     │   │   控制器層   │   │   服務層     │   │   模型層   │  │
│   └──────────────┘   └──────────────┘   └──────┬───────┘   └──────┬─────┘  │
│                                                 │                  │        │
│   主要服務:                                     │                  │        │
│   • forexService (市場數據)                     │                  │        │
│   • tradingSignalService (信號生成)            │                  │        │
│   • signalMonitoringService (信號監控)         │                  │        │
│   • mlEngineService (ML 整合)                  │                  │        │
│                                                 │                  │        │
└────────────────────────────────────────────────┼──────────────────┼────────┘
                                                  │                  │
          ┌───────────────────────────────────────┼──────────────────┼───────┐
          │                                       │                  │       │
          ▼                                       ▼                  ▼       │
┌─────────────────┐                    ┌─────────────────┐  ┌──────────────┐ │
│   ML Engine     │                    │   PostgreSQL    │  │    Redis     │ │
│   (FastAPI)     │                    │   Database      │  │    Cache     │ │
│   Port: 8000    │                    │   Port: 5432    │  │  Port: 6379  │ │
├─────────────────┤                    ├─────────────────┤  ├──────────────┤ │
│ • LSTM Model    │                    │ • users         │  │ • 市場數據   │ │
│ • FinBERT       │                    │ • market_data   │  │ • 信號快取   │ │
│ • VADER         │                    │ • signals       │  │ • Session    │ │
│ • Reversal API  │                    │ • subscriptions │  │ • Pub/Sub    │ │
│ • A/B Testing   │                    │ • ...           │  │              │ │
└─────────────────┘                    └─────────────────┘  └──────────────┘ │
                                                                              │
┌─────────────────────────────────────────────────────────────────────────────┘
│
│   外部數據源:
│   ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   │   Twelve Data   │    │   Google News   │    │    NewsAPI      │
│   │      API        │    │      RSS        │    │                 │
│   │  (市場數據)     │    │  (新聞情緒)     │    │  (新聞情緒)     │
│   └─────────────────┘    └─────────────────┘    └─────────────────┘
│
└─────────────────────────────────────────────────────────────────────────────
```

### 2.2 服務端口配置

| 服務 | 端口 | 說明 |
|------|------|------|
| Backend API | 3000 | 核心 REST API 服務 |
| ML Engine | 8000 | Python FastAPI ML 服務 |
| LINE Bot | 3001 | LINE Webhook 伺服器 |
| PostgreSQL | 5432 | 關聯式資料庫 |
| Redis | 6379 | 快取與 Pub/Sub |
| Frontend (Dev) | 5173 | React 開發伺服器 |

---

## 3. 技術棧

### 3.1 後端 (Backend)

| 技術 | 版本 | 用途 |
|------|------|------|
| Node.js | ≥ 18.0.0 | JavaScript 執行環境 |
| Express.js | ^4.18.0 | Web 框架 |
| Sequelize | ^6.0.0 | ORM (PostgreSQL) |
| jsonwebtoken | ^9.0.0 | JWT 認證 |
| bcrypt | ^5.0.0 | 密碼雜湊 |
| axios | ^1.0.0 | HTTP 客戶端 |
| socket.io | ^4.0.0 | WebSocket |
| redis | ^4.0.0 | Redis 客戶端 |
| winston | ^3.0.0 | 日誌記錄 |
| joi | ^17.0.0 | 數據驗證 |
| helmet | ^7.0.0 | 安全標頭 |
| cors | ^2.8.0 | CORS 處理 |

### 3.2 機器學習引擎 (ML Engine)

| 技術 | 版本 | 用途 |
|------|------|------|
| Python | ≥ 3.9 | 程式語言 |
| TensorFlow | ≥ 2.10.0 | 深度學習框架 |
| scikit-learn | ≥ 1.0.0 | 機器學習工具 |
| pandas | ≥ 1.5.0 | 數據處理 |
| numpy | ≥ 1.23.0 | 數值運算 |
| FastAPI | ≥ 0.100.0 | Web API 框架 |
| uvicorn | ≥ 0.23.0 | ASGI 伺服器 |
| transformers | latest | FinBERT 模型 |
| vaderSentiment | latest | VADER 情緒分析 |

### 3.3 前端 (Frontend)

| 技術 | 版本 | 用途 |
|------|------|------|
| React | ^18.2.0 | UI 框架 |
| Vite | ^4.0.0 | 建置工具 |
| Tailwind CSS | ^3.0.0 | CSS 框架 |
| Chart.js | ^4.0.0 | 圖表繪製 |
| socket.io-client | ^4.0.0 | WebSocket 客戶端 |
| axios | ^1.0.0 | HTTP 客戶端 |

### 3.4 Bot 服務

| 技術 | 版本 | 用途 |
|------|------|------|
| Discord.js | ^14.0.0 | Discord Bot SDK |
| @line/bot-sdk | ^8.0.0 | LINE Bot SDK |

### 3.5 資料庫與快取

| 技術 | 版本 | 用途 |
|------|------|------|
| PostgreSQL | ≥ 14.0 | 主資料庫 |
| Redis | ≥ 6.0 | 快取/Pub/Sub |

---

## 4. 微服務架構

### 4.1 核心原則

AIFX v2 遵循微服務架構，確保各服務獨立運作：

#### 1️⃣ 服務獨立性

```
✅ 每個服務可以獨立啟動/停止
✅ 服務故障不會連鎖影響其他服務
✅ 每個服務有獨立的健康檢查端點
❌ 服務間不共享資料庫模型
❌ 除 Backend 外，其他服務不直接存取資料庫
```

#### 2️⃣ 單一資料庫擁有者

```
┌─────────────────────────────────────────────────────────────┐
│                   資料庫存取策略                             │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│   Frontend          ──┐                                      │
│                       │                                      │
│   Discord Bot       ──┼──── REST API ────► Backend ──► PostgreSQL
│                       │                      (唯一)          │
│   LINE Bot          ──┼                                      │
│                       │                                      │
│   ML Engine         ──┘                                      │
│                                                              │
│   ✅ 只有 Backend 可直接存取 PostgreSQL                      │
│   ✅ 其他服務必須透過 Backend REST API                       │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

#### 3️⃣ API 通訊規則

```
服務間通訊架構:

Frontend ─── JWT Token ──────────┐
                                  │
Discord Bot ─ X-API-Key ─────────┼───► Backend ───► PostgreSQL
                                  │        │
LINE Bot ─── Bearer Token ───────┘        ├───► Redis
                                           │
Admin App ─── Admin JWT ─────────────────►│
                                           │
                                           └───► ML Engine
```

### 4.2 服務責任矩陣

| 責任 | Backend | ML Engine | Discord Bot | LINE Bot | Frontend |
|------|---------|-----------|-------------|----------|----------|
| 資料庫存取 | ✅ 唯一 | ❌ API | ❌ API | ❌ API | ❌ API |
| 用戶認證 | ✅ | ❌ | ❌ | ❌ | ✅ Client |
| ML 訓練 | ❌ | ✅ 唯一 | ❌ | ❌ | ❌ |
| Discord 訊息 | ❌ | ❌ | ✅ 唯一 | ❌ | ❌ |
| LINE 訊息 | ❌ | ❌ | ❌ | ✅ 唯一 | ❌ |
| WebSocket | ✅ Server | ❌ | ❌ | ❌ | ✅ Client |
| 信號生成 | ✅ | ✅ 預測 | ❌ | ❌ | ❌ |

### 4.3 健康檢查端點

每個服務都必須實作健康檢查：

```javascript
// 回應格式
GET /health

{
  "status": "healthy",
  "service": "backend",
  "version": "1.0.0",
  "uptime": 3600,
  "dependencies": {
    "postgres": "connected",
    "redis": "connected",
    "ml_engine": "connected"
  }
}
```

---

## 5. Backend API 服務

### 5.1 架構概述

```
┌─────────────────────────────────────────────────────────────────────────┐
│                      Backend API (Port 3000)                             │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   Request ──► Middleware ──► Routes ──► Controllers ──► Services        │
│                  │                                          │            │
│                  │                                          ▼            │
│           ┌──────┴──────┐                            ┌──────────────┐   │
│           │ • auth.js   │                            │    Models    │   │
│           │ • cors      │                            │  (Sequelize) │   │
│           │ • rateLimit │                            └──────┬───────┘   │
│           │ • helmet    │                                   │           │
│           └─────────────┘                                   ▼           │
│                                                       ┌──────────────┐   │
│                                                       │  PostgreSQL  │   │
│                                                       └──────────────┘   │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### 5.2 目錄結構

```
backend/src/
├── app.js                    # Express 應用程式配置
├── server.js                 # 伺服器入口點
├── config/
│   ├── database.js           # PostgreSQL 配置
│   └── redis.js              # Redis 配置
├── controllers/
│   ├── adminController.js    # 管理員控制器
│   ├── authController.js     # 認證控制器
│   ├── tradingController.js  # 交易訊號控制器
│   ├── marketController.js   # 市場數據控制器
│   └── subscriptionsController.js  # 訂閱控制器
├── services/
│   ├── forexService.js       # 外匯數據服務
│   ├── tradingSignalService.js    # 訊號生成服務
│   ├── signalMonitoringService.js # 信號監控服務
│   ├── mlEngineService.js    # ML Engine 整合服務
│   └── technicalAnalysis.js  # 技術指標計算
├── models/
│   ├── index.js              # 模型關聯定義
│   ├── User.js               # 用戶模型
│   ├── UserDiscordSettings.js
│   ├── UserLineSettings.js
│   ├── TradingSignal.js
│   ├── MarketData.js
│   ├── UserSubscription.js
│   └── ...
├── routes/
│   ├── auth.js               # 認證路由
│   ├── trading.js            # 交易路由
│   ├── market.js             # 市場數據路由
│   ├── notifications.js      # 通知路由
│   ├── admin.js              # 管理員路由
│   └── api/v1/
│       ├── discord/          # Discord Bot API
│       ├── line/             # LINE Bot API
│       ├── ml/               # ML 相關 API
│       └── subscriptions.js  # 訂閱 API
├── middleware/
│   ├── auth.js               # JWT 認證中間件
│   └── adminAuth.js          # 管理員認證中間件
└── utils/
    ├── periodMapper.js       # 交易週期映射
    └── logger.js             # 日誌工具
```

### 5.3 核心服務詳解

#### forexService.js - 市場數據服務

```javascript
/**
 * 功能:
 * - 獲取即時/歷史市場數據
 * - Twelve Data API 整合
 * - Redis 快取管理 (TTL: 60 秒)
 * - 混合模式: 資料庫 99 根 + API 1 根最新
 *
 * 主要方法:
 * - getMarketData(pair, timeframe)
 * - getHistoricalData(pair, timeframe, limit)
 * - cacheData(key, data, ttl)
 */
```

#### tradingSignalService.js - 訊號生成服務

```javascript
/**
 * 功能:
 * - 生成交易訊號 (BUY/SELL/HOLD)
 * - 整合 ML Engine 預測
 * - 計算技術指標 (SMA, RSI, MACD)
 * - 融合技術分析與情緒分析
 *
 * 主要方法:
 * - generateSignal(pair, timeframe)
 * - calculateIndicators(data)
 * - fuseSignals(technical, sentiment, timeframe)
 */
```

#### signalMonitoringService.js - 信號監控服務

```javascript
/**
 * 功能:
 * - 定時監控信號變化 (每 15 分鐘)
 * - 比對新舊信號狀態
 * - 發布 Redis Pub/Sub 事件
 * - 通知訂閱用戶
 *
 * 主要方法:
 * - startMonitoring()
 * - checkSignalChanges()
 * - notifySubscribers(event)
 */
```

### 5.4 API 回應格式

所有 API 遵循統一回應格式：

```javascript
// 成功回應
{
  "success": true,
  "data": {
    // 實際數據
  },
  "error": null,
  "timestamp": "2025-12-02T10:30:00Z"
}

// 錯誤回應
{
  "success": false,
  "data": null,
  "error": {
    "code": "INVALID_PAIR",
    "message": "Invalid currency pair format"
  },
  "timestamp": "2025-12-02T10:30:00Z"
}
```

---

## 6. ML Engine 機器學習引擎

### 6.1 架構概述

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        ML Engine (Port 8000)                             │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   ┌─────────────────────────────────────────────────────────────────┐   │
│   │                      FastAPI Application                         │   │
│   │                                                                  │   │
│   │   /health        → 健康檢查                                      │   │
│   │   /predict       → LSTM 價格預測                                 │   │
│   │   /train         → 模型訓練                                      │   │
│   │   /sentiment/*   → 情緒分析                                      │   │
│   │   /reversal/*    → 反轉預測                                      │   │
│   │   /model/info    → 模型資訊                                      │   │
│   │   /market-data/* → 市場數據                                      │   │
│   │                                                                  │   │
│   └─────────────────────────────────────────────────────────────────┘   │
│                                                                          │
│   ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐        │
│   │  Price         │  │   Sentiment     │  │   Reversal      │        │
│   │  Predictor     │  │   Analyzer      │  │   Predictor     │        │
│   │  (LSTM)        │  │  (FinBERT)      │  │  (2-Stage)      │        │
│   └────────┬────────┘  └────────┬────────┘  └────────┬────────┘        │
│            │                    │                    │                  │
│            └────────────────────┼────────────────────┘                  │
│                                 │                                       │
│                    ┌────────────▼────────────┐                         │
│                    │    Signal Fusion        │                         │
│                    │   (加權融合決策)        │                         │
│                    └─────────────────────────┘                         │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### 6.2 LSTM 價格預測模型

#### 模型架構

```
輸入層 (60 時間步 × 12 特徵)
         │
         ▼
┌─────────────────────────────┐
│     LSTM Layer 1            │
│     (128 units)             │
│     return_sequences=True   │
│     dropout=0.2             │
└─────────────┬───────────────┘
              ▼
┌─────────────────────────────┐
│     LSTM Layer 2            │
│     (64 units)              │
│     return_sequences=False  │
│     dropout=0.2             │
└─────────────┬───────────────┘
              ▼
┌─────────────────────────────┐
│     Dropout (0.2)           │
└─────────────┬───────────────┘
              ▼
┌─────────────────────────────┐
│     Dense (32, ReLU)        │
└─────────────┬───────────────┘
              ▼
┌─────────────────────────────┐
│  Dense (3, Softmax)         │
│  [Long, Short, Standby]     │
└─────────────────────────────┘
```

#### 輸入特徵 (12 維)

| # | 特徵 | 說明 | 計算方式 |
|---|------|------|----------|
| 1 | Open | 開盤價 | 原始數據 |
| 2 | High | 最高價 | 原始數據 |
| 3 | Low | 最低價 | 原始數據 |
| 4 | Close | 收盤價 | 原始數據 |
| 5 | Volume | 成交量 | 原始數據 |
| 6 | SMA_5 | 5 期簡單移動平均 | (C₁+C₂+...+C₅)/5 |
| 7 | SMA_20 | 20 期簡單移動平均 | (C₁+C₂+...+C₂₀)/20 |
| 8 | EMA_12 | 12 期指數移動平均 | 加權移動平均 |
| 9 | RSI_14 | 14 期相對強弱指標 | 100 - 100/(1+RS) |
| 10 | MACD | MACD 線 | EMA₁₂ - EMA₂₆ |
| 11 | BB_Upper | 布林通道上軌 | SMA₂₀ + 2σ |
| 12 | BB_Lower | 布林通道下軌 | SMA₂₀ - 2σ |

#### 訓練參數

```python
optimizer = Adam(learning_rate=0.001)
loss = 'categorical_crossentropy'
batch_size = 32
epochs = 100
early_stopping = EarlyStopping(
    monitor='val_loss',
    patience=10,
    restore_best_weights=True
)
```

### 6.3 情緒分析系統

#### 分析來源

| 來源 | 模型/方法 | 說明 |
|------|----------|------|
| 新聞情緒 | FinBERT | 金融專用 BERT 模型 (ProsusAI/finbert) |
| 央行政策 | VADER + 關鍵字 | 貨幣政策立場分析 |
| Google News | RSS Feed | 即時新聞抓取 (免費備用方案) |

#### FinBERT 情緒分析流程

```python
# 1. 載入模型
model = pipeline("sentiment-analysis", model="ProsusAI/finbert")

# 2. 分析新聞標題與內容
result = model(text[:512])
# 輸出: {'label': 'positive/negative/neutral', 'score': 0.0-1.0}

# 3. 映射到情緒分數
score_map = {
    'positive': 0.8,  # 看多
    'negative': 0.2,  # 看空
    'neutral': 0.5    # 中性
}
```

#### 央行政策分析

| 貨幣 | 央行 | 分析關鍵字 |
|------|------|-----------|
| USD | Federal Reserve (Fed) | interest rate, FOMC, hawkish, dovish |
| EUR | European Central Bank (ECB) | rate decision, monetary policy |
| JPY | Bank of Japan (BOJ) | yield curve control, stimulus |
| GBP | Bank of England (BoE) | inflation report, rate decision |

#### 情緒分數計算

```python
# 新聞情緒分數 (-1 到 +1 → 0 到 1)
news_sentiment = weighted_average(article_scores)

# 央行情緒分數
central_bank_sentiment = analyze_policy_stance(policy_text)

# 自適應權重 (根據時間框架)
weights = calculate_adaptive_weights(news, gov, timeframe)

# 綜合情緒分數
combined_sentiment = (news_sentiment * weights['news'] +
                      central_bank_sentiment * weights['gov'])
```

### 6.4 訊號融合機制

#### 時間框架權重

| 時間框架 | 技術權重 | 情緒權重 | 說明 |
|----------|----------|----------|------|
| 15min | 95% | 5% | 短線以技術為主 |
| 1h | 85% | 15% | 波段交易 |
| 4h | 70% | 30% | 中期交易 |
| 1d | 55% | 45% | 日線重視基本面 |
| 1w | 40% | 60% | 長線情緒主導 |

#### 融合公式

```python
# 技術分數 (LSTM 輸出)
technical_score = lstm_prediction['confidence']

# 情緒分數
sentiment_score = sentiment_analyzer.analyze(pair)['sentiment_score']

# 取得權重
tech_weight, sent_weight = get_weights(timeframe)

# 最終信心度
final_confidence = (technical_score * tech_weight) + (sentiment_score * sent_weight)

# 訊號強度判定
if final_confidence > 0.75:
    strength = 'very_strong'
elif final_confidence > 0.65:
    strength = 'strong'
elif final_confidence > 0.55:
    strength = 'moderate'
else:
    strength = 'weak'
```

### 6.5 API 端點

| 端點 | 方法 | 說明 |
|------|------|------|
| `/health` | GET | 健康檢查 |
| `/predict` | POST | LSTM 價格預測 |
| `/train` | POST | 模型訓練 |
| `/model/info` | GET | 模型資訊 |
| `/market-data/{pair}` | GET | 獲取市場數據 |
| `/sentiment/analyze` | POST | 情緒分析 |
| `/sentiment/health` | GET | 情緒服務健康檢查 |
| `/reversal/predict_raw` | POST | 反轉預測 (含情緒) |

---

## 7. Discord Bot

### 7.1 功能概述

Discord Bot 提供斜線命令介面，讓用戶透過 Discord 查詢交易訊號與管理訂閱。

### 7.2 斜線命令列表

| 命令 | 參數 | 說明 |
|------|------|------|
| `/signal` | `[貨幣對]` `[週期]` | 查詢交易訊號 |
| `/subscribe` | `[貨幣對]` `[時間框架]` | 訂閱信號變化通知 |
| `/unsubscribe` | `[貨幣對]` `[時間框架]` | 取消訂閱 |
| `/subscriptions` | - | 查看所有訂閱 |
| `/position` | `[貨幣對]` | 查看持倉建議 |
| `/preferences` | - | 設定交易偏好 |
| `/trading-guide` | - | 顯示交易指南 |
| `/ping` | - | 測試 Bot 連線 |

### 7.3 技術實作

```javascript
// bot.js 主要流程

// 1. 初始化 Discord Client
const client = new Client({
  intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages]
});

// 2. 載入斜線命令
function loadCommands() {
  const commandFiles = fs.readdirSync('./commands');
  for (const file of commandFiles) {
    const command = require(`./commands/${file}`);
    client.commands.set(command.data.name, command);
  }
}

// 3. 監聽互動事件
client.on(Events.InteractionCreate, async interaction => {
  const command = client.commands.get(interaction.commandName);
  await command.execute(interaction);
});

// 4. Redis Pub/Sub 監聽信號變化
redisSubscriber.subscribe('signal-change', async (message) => {
  const event = JSON.parse(message);
  await handleSignalChangeNotification(event);
});
```

### 7.4 Embed 訊息格式

```
┌────────────────────────────────────────┐
│ 📊 AIFX 交易訊號                        │
├────────────────────────────────────────┤
│ 貨幣對：EUR/USD                         │
│ 方向：🟢 買入 (LONG)                    │
│ 信心度：75%                             │
│ 強度：STRONG                            │
│ 週期：周內 (1H)                         │
│ 市場狀況：TRENDING                      │
├────────────────────────────────────────┤
│ 💰 入場價格: 1.08500                    │
│ 🛑 停損價格: 1.08200                    │
│ 🎯 目標價格: 1.09000                    │
├────────────────────────────────────────┤
│ ⚠️ 僅供參考，請自行評估風險             │
└────────────────────────────────────────┘
```

---

## 8. LINE Bot

### 8.1 功能概述

LINE Bot 透過文字訊息指令與 Flex Message 提供交易訊號查詢與訂閱管理。

### 8.2 文字指令列表

| 指令 | 範例 | 說明 |
|------|------|------|
| 查詢訊號 | `EUR/USD` | 查詢預設週期訊號 |
| 查詢特定週期 | `EUR/USD 周內` | 查詢指定週期訊號 |
| 訂閱 | `訂閱 EUR/USD` | 訂閱信號變化通知 |
| 訂閱指定框架 | `訂閱 EUR/USD 1h` | 訂閱指定時間框架 |
| 取消訂閱 | `取消訂閱 EUR/USD` | 取消訂閱 |
| 查看訂閱 | `我的訂閱` | 查看所有訂閱 |
| 幫助 | `幫助` | 顯示指令說明 |

**支援中英文指令：**
- 中文：訂閱、取消訂閱、我的訂閱、幫助
- 英文：subscribe、unsubscribe、subscriptions、help

### 8.3 技術實作

```javascript
// bot.js 主要架構

// 1. Express Webhook 伺服器
const app = express();
app.post('/webhook', line.middleware(config), async (req, res) => {
  await Promise.all(req.body.events.map(handleEvent));
  res.json({ success: true });
});

// 2. 事件處理
async function handleEvent(event) {
  switch (event.type) {
    case 'message':
      if (event.message.type === 'text') {
        await messageHandler.handleText(event, client);
      }
      break;
    case 'follow':
      await messageHandler.handleFollow(event, client);
      break;
    // ...
  }
}

// 3. Redis Pub/Sub 監聽
redisSubscriber.subscribe('signal-change', async (message) => {
  const event = JSON.parse(message);
  await handleSignalChangeNotification(event);
});
```

### 8.4 Flex Message 格式

```
┌────────────────────────────────────────┐
│ 📊 AIFX 交易訊號                        │
├────────────────────────────────────────┤
│ EUR/USD                                 │
│ ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  │
│ 🟢 買入 (LONG)           信心度 75%    │
│                                         │
│ 週期：周內 (1H)                         │
│ 強度：STRONG                            │
│ 市場：TRENDING                          │
├────────────────────────────────────────┤
│ 📈 技術指標                             │
│ SMA: 1.0850  RSI: 45                   │
├────────────────────────────────────────┤
│ ⚠️ 僅供參考，請自行評估風險             │
└────────────────────────────────────────┘
```

---

## 9. 資料庫設計

### 9.1 ER 關係圖

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                           資料庫關聯圖                                        │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                               │
│   ┌─────────────────┐                                                        │
│   │     users       │◄────────────────────────────────────┐                  │
│   ├─────────────────┤                                     │                  │
│   │ id (PK)         │     ┌───────────────────────┐       │                  │
│   │ email           │     │ user_discord_settings │       │                  │
│   │ username        │     ├───────────────────────┤       │                  │
│   │ password_hash   │     │ id (PK)               │       │                  │
│   │ created_at      │◄────│ user_id (FK)          │       │                  │
│   │ updated_at      │     │ discord_id            │       │                  │
│   └────────┬────────┘     │ notification_enabled  │       │                  │
│            │              └───────────────────────┘       │                  │
│            │                                               │                  │
│            │              ┌───────────────────────┐       │                  │
│            │              │  user_line_settings   │       │                  │
│            │              ├───────────────────────┤       │                  │
│            └─────────────►│ id (PK)               │       │                  │
│                           │ user_id (FK)          │       │                  │
│                           │ line_user_id          │       │                  │
│                           │ display_name          │       │                  │
│                           └───────────────────────┘       │                  │
│                                                           │                  │
│   ┌─────────────────┐     ┌───────────────────────┐       │                  │
│   │  market_data    │     │  user_subscriptions   │       │                  │
│   ├─────────────────┤     ├───────────────────────┤       │                  │
│   │ id (PK)         │     │ id (PK)               │       │                  │
│   │ pair            │     │ user_id (FK)          │───────┘                  │
│   │ timeframe       │     │ pair                  │                          │
│   │ open            │     │ timeframe             │                          │
│   │ high            │     │ platform              │                          │
│   │ low             │     │ is_active             │                          │
│   │ close           │     │ created_at            │                          │
│   │ volume          │     └───────────────────────┘                          │
│   │ timestamp       │                                                        │
│   └─────────────────┘     ┌───────────────────────┐                          │
│                           │   trading_signals     │                          │
│                           ├───────────────────────┤                          │
│                           │ id (PK)               │                          │
│                           │ pair                  │                          │
│                           │ timeframe             │                          │
│                           │ signal                │                          │
│                           │ confidence            │                          │
│                           │ signal_strength       │                          │
│                           │ entry_price           │                          │
│                           │ stop_loss             │                          │
│                           │ take_profit           │                          │
│                           │ created_at            │                          │
│                           └───────────────────────┘                          │
│                                                                               │
└──────────────────────────────────────────────────────────────────────────────┘
```

### 9.2 資料表定義

#### users (用戶表)

```sql
CREATE TABLE users (
  id SERIAL PRIMARY KEY,
  email VARCHAR(255) UNIQUE NOT NULL,
  username VARCHAR(100) UNIQUE NOT NULL,
  password_hash VARCHAR(255) NOT NULL,
  is_active BOOLEAN DEFAULT true,
  is_verified BOOLEAN DEFAULT false,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);
```

#### user_discord_settings (Discord 用戶設定)

```sql
CREATE TABLE user_discord_settings (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  discord_id VARCHAR(50) UNIQUE NOT NULL,
  discord_username VARCHAR(100),
  notification_enabled BOOLEAN DEFAULT true,
  preferred_pairs TEXT[],
  risk_level INTEGER DEFAULT 5,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);
```

#### user_line_settings (LINE 用戶設定)

```sql
CREATE TABLE user_line_settings (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  line_user_id VARCHAR(50) UNIQUE NOT NULL,
  display_name VARCHAR(100),
  notification_enabled BOOLEAN DEFAULT true,
  preferred_pairs TEXT[],
  risk_level INTEGER DEFAULT 5,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);
```

#### market_data (市場數據)

```sql
CREATE TABLE market_data (
  id SERIAL PRIMARY KEY,
  pair VARCHAR(10) NOT NULL,
  timeframe VARCHAR(10) NOT NULL,
  open DECIMAL(20, 10) NOT NULL,
  high DECIMAL(20, 10) NOT NULL,
  low DECIMAL(20, 10) NOT NULL,
  close DECIMAL(20, 10) NOT NULL,
  volume DECIMAL(20, 4),
  timestamp TIMESTAMP NOT NULL,
  created_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(pair, timeframe, timestamp)
);

CREATE INDEX idx_market_data_pair_timeframe ON market_data(pair, timeframe);
CREATE INDEX idx_market_data_timestamp ON market_data(timestamp DESC);
```

#### trading_signals (交易訊號)

```sql
CREATE TABLE trading_signals (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id),
  pair VARCHAR(10) NOT NULL,
  timeframe VARCHAR(10) NOT NULL,
  signal VARCHAR(10) NOT NULL,  -- 'buy', 'sell', 'hold', 'standby'
  confidence DECIMAL(5, 4),
  signal_strength VARCHAR(20),  -- 'weak', 'moderate', 'strong', 'very_strong'
  market_condition VARCHAR(20), -- 'trending', 'ranging', 'volatile'
  entry_price DECIMAL(20, 10),
  stop_loss DECIMAL(20, 10),
  take_profit DECIMAL(20, 10),
  indicators JSONB,
  sentiment_data JSONB,
  created_at TIMESTAMP DEFAULT NOW()
);
```

#### user_subscriptions (用戶訂閱)

```sql
CREATE TABLE user_subscriptions (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  pair VARCHAR(10) NOT NULL,
  timeframe VARCHAR(10) NOT NULL,
  platform VARCHAR(20) NOT NULL,  -- 'discord' or 'line'
  is_active BOOLEAN DEFAULT true,
  last_notified_at TIMESTAMP,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, pair, timeframe, platform)
);
```

---

## 10. 訂閱通知系統

### 10.1 系統架構

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         訂閱通知系統架構                                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                  Signal Monitoring Service                           │   │
│   │                    (每 15 分鐘執行)                                  │   │
│   │                                                                      │   │
│   │   1. 獲取所有貨幣對的最新訊號                                        │   │
│   │   2. 比對 Redis 快取中的舊訊號                                       │   │
│   │   3. 偵測信號變化                                                    │   │
│   │   4. 查詢該貨幣對的訂閱用戶                                          │   │
│   │   5. 發布 signal-change 事件                                         │   │
│   │                                                                      │   │
│   └────────────────────────────────┬────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
│   ┌────────────────────────────────────────────────────────────────────┐    │
│   │                     Redis Pub/Sub                                   │    │
│   │                  Channel: 'signal-change'                           │    │
│   │                                                                     │    │
│   │   Event Format:                                                     │    │
│   │   {                                                                 │    │
│   │     "pair": "EUR/USD",                                             │    │
│   │     "timeframe": "1h",                                             │    │
│   │     "oldSignal": "hold",                                           │    │
│   │     "newSignal": "buy",                                            │    │
│   │     "newConfidence": 0.75,                                         │    │
│   │     "signalStrength": "strong",                                    │    │
│   │     "marketCondition": "trending",                                 │    │
│   │     "entryPrice": 1.0850,                                          │    │
│   │     "subscribers": [                                               │    │
│   │       { "platform": "discord", "id": "123456789" },               │    │
│   │       { "platform": "line", "id": "U1234567890" }                 │    │
│   │     ]                                                              │    │
│   │   }                                                                │    │
│   │                                                                     │    │
│   └────────────────┬───────────────────────────────┬───────────────────┘    │
│                    │                               │                        │
│                    ▼                               ▼                        │
│   ┌────────────────────────────────┐  ┌────────────────────────────────┐   │
│   │        Discord Bot             │  │         LINE Bot               │   │
│   │                                │  │                                │   │
│   │  • 接收 signal-change 事件     │  │  • 接收 signal-change 事件     │   │
│   │  • 過濾 platform='discord'     │  │  • 過濾 platform='line'        │   │
│   │  • 發送 Embed 通知訊息         │  │  • 發送 Flex Message           │   │
│   │                                │  │                                │   │
│   └────────────────────────────────┘  └────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 10.2 信號變化偵測邏輯

觸發通知的信號變化：

| 變化類型 | 說明 |
|----------|------|
| BUY ↔ SELL | 買入與賣出互換 |
| BUY ↔ HOLD | 買入與觀望互換 |
| SELL ↔ HOLD | 賣出與觀望互換 |
| 任何 → STANDBY | 進入待命狀態 |
| STANDBY → 任何 | 退出待命狀態 |

### 10.3 冷卻機制

- 同一用戶/貨幣對/時間框架，**30 分鐘**內只發送一次通知
- 防止過度通知干擾用戶
- 冷卻記錄存儲於 Redis (TTL: 30 分鐘)

---

## 11. 交易週期設計

### 11.1 週期對照表

| 週期 | 中文 | 英文 | 時間框架 | 適用交易者 | 持倉時間 |
|------|------|------|----------|-----------|----------|
| 日內 | 日內交易 | Intraday | 15min | 當沖客 | 數小時 |
| 周內 | 波段交易 | Swing | 1h | 波段交易者 | 1-5 天 |
| 月內 | 趨勢跟隨 | Position | 1d | 中期投資者 | 1-4 週 |
| 季內 | 長期投資 | Long-term | 1w | 長期投資者 | 1-3 個月 |

### 11.2 週期映射邏輯

```javascript
// backend/src/utils/periodMapper.js

const PERIOD_MAPPING = {
  // 中文週期名稱
  '日內': {
    timeframe: '15min',
    english: 'intraday',
    description: '當天平倉',
    holdingPeriod: '數小時',
    technicalWeight: 0.95,
    sentimentWeight: 0.05
  },
  '周內': {
    timeframe: '1h',
    english: 'swing',
    description: '波段操作',
    holdingPeriod: '1-5天',
    technicalWeight: 0.85,
    sentimentWeight: 0.15
  },
  '月內': {
    timeframe: '1d',
    english: 'position',
    description: '趨勢跟隨',
    holdingPeriod: '1-4週',
    technicalWeight: 0.55,
    sentimentWeight: 0.45
  },
  '季內': {
    timeframe: '1w',
    english: 'longterm',
    description: '長期持有',
    holdingPeriod: '1-3個月',
    technicalWeight: 0.40,
    sentimentWeight: 0.60
  }
};

// 英文別名映射
const ENGLISH_ALIASES = {
  'intraday': '日內',
  'daytrading': '日內',
  'swing': '周內',
  'position': '月內',
  'longterm': '季內',
  'long-term': '季內'
};
```

### 11.3 API 使用範例

```bash
# 中文參數
GET /api/v1/trading/signal?pair=EUR/USD&period=日內

# 英文參數
GET /api/v1/trading/signal?pair=EUR/USD&period=intraday

# 直接使用時間框架
GET /api/v1/trading/signal?pair=EUR/USD&timeframe=15min
```

---

## 12. API 端點完整列表

### 12.1 Backend API (Port 3000)

#### 公開路由 (無需認證)

| 路由 | 方法 | 說明 |
|------|------|------|
| `/health` | GET | 健康檢查 |
| `/api/v1/auth/login` | POST | 用戶登入 |
| `/api/v1/auth/register` | POST | 用戶註冊 |

#### 交易 API (需 JWT)

| 路由 | 方法 | 說明 |
|------|------|------|
| `/api/v1/trading/signal` | GET | 查詢交易訊號 |
| `/api/v1/trading/signals` | GET | 列出所有訊號 |

#### Discord Bot API (需 X-API-Key)

| 路由 | 方法 | 說明 |
|------|------|------|
| `/api/v1/discord/signals` | GET | 獲取所有訊號 |
| `/api/v1/discord/signals/:pair` | GET | 獲取特定貨幣對訊號 |
| `/api/v1/discord/users/:discordId` | GET | 獲取用戶資訊 |
| `/api/v1/discord/users` | POST | 建立/更新用戶 |

#### LINE Bot API (需 Bearer Token)

| 路由 | 方法 | 說明 |
|------|------|------|
| `/api/v1/line/signals` | GET | 獲取所有訊號 |
| `/api/v1/line/signals/:pair` | GET | 獲取特定貨幣對訊號 |
| `/api/v1/line/users/:lineUserId` | GET | 獲取用戶資訊 |
| `/api/v1/line/users` | POST | 建立/更新用戶 |
| `/api/v1/line/subscriptions` | POST | 建立訂閱 |
| `/api/v1/line/subscriptions/:lineUserId` | GET | 獲取用戶訂閱 |

#### 訂閱 API

| 路由 | 方法 | 說明 |
|------|------|------|
| `/api/v1/notifications/subscribe` | POST | 訂閱通知 |
| `/api/v1/notifications/unsubscribe` | POST | 取消訂閱 |
| `/api/v1/notifications/subscriptions/:userId` | GET | 獲取訂閱列表 |

#### 市場數據 API

| 路由 | 方法 | 說明 |
|------|------|------|
| `/api/v1/market/history/:pair` | GET | 獲取歷史數據 |
| `/api/v1/market/data/bulk` | POST | 批量寫入數據 (內部) |

#### Admin API (需 Admin JWT)

| 路由 | 方法 | 說明 |
|------|------|------|
| `/api/v1/admin/login` | POST | 管理員登入 |
| `/api/v1/admin/verify` | GET | 驗證 Token |
| `/api/v1/admin/health` | GET | 系統健康狀態 |
| `/api/v1/admin/stats` | GET | 統計資訊 |
| `/api/v1/admin/users` | GET | 用戶列表 |
| `/api/v1/admin/signals` | GET | 訊號列表 |
| `/api/v1/admin/ml/status` | GET | ML Engine 狀態 |
| `/api/v1/admin/sentiment/test/:pair` | GET | 測試情緒分析 |

### 12.2 ML Engine API (Port 8000)

| 路由 | 方法 | 說明 |
|------|------|------|
| `/health` | GET | 健康檢查 |
| `/predict` | POST | LSTM 價格預測 |
| `/train` | POST | 模型訓練 |
| `/model/info` | GET | 模型資訊 |
| `/market-data/{pair}` | GET | 獲取市場數據 |
| `/sentiment/analyze` | POST | 情緒分析 |
| `/sentiment/health` | GET | 情緒服務健康檢查 |
| `/reversal/predict_raw` | POST | 反轉預測 |
| `/docs` | GET | Swagger API 文件 |

---

## 13. 認證與安全機制

### 13.1 認證類型對照

| 認證類型 | Header | 格式 | 使用者 | 說明 |
|----------|--------|------|--------|------|
| API Key | X-API-Key | 64 字元 hex | Discord Bot | 服務間認證 |
| Bearer Token | Authorization | Bearer + 64 字元 | LINE Bot | 服務間認證 |
| JWT Token | Authorization | Bearer + JWT | 前端用戶 | 用戶認證 |
| Admin Token | Authorization | Bearer + JWT | 管理員 | 管理後台認證 |

### 13.2 認證中間件邏輯

```javascript
// middleware/auth.js

async function authenticate(req, res, next) {
  const authHeader = req.headers.authorization;
  const apiKey = req.headers['x-api-key'];

  // 1. 檢查 X-API-Key (Discord Bot)
  if (apiKey) {
    if (apiKey === process.env.API_KEY) {
      req.user = { type: 'discord-bot' };
      return next();
    }
    return res.status(401).json({ error: 'Invalid API Key' });
  }

  // 2. 檢查 Bearer Token
  if (authHeader && authHeader.startsWith('Bearer ')) {
    const token = authHeader.substring(7);

    // 2a. 64 字元 hex (LINE Bot API Key)
    if (token.length === 64 && !token.includes('.')) {
      if (token === process.env.LINE_BOT_API_KEY) {
        req.user = { type: 'line-bot' };
        return next();
      }
      return res.status(401).json({ error: 'Invalid API Key' });
    }

    // 2b. JWT Token (用戶/管理員)
    try {
      const decoded = jwt.verify(token, process.env.JWT_SECRET);
      req.user = decoded;
      return next();
    } catch (err) {
      return res.status(401).json({ error: 'Invalid token' });
    }
  }

  return res.status(401).json({ error: 'Authentication required' });
}
```

### 13.3 安全最佳實踐

| 項目 | 實作方式 |
|------|----------|
| 密碼雜湊 | bcrypt (10 rounds) |
| JWT 過期 | Access: 15分鐘, Refresh: 7天 |
| API 速率限制 | 100 req/min/IP |
| CORS | 限制允許的來源 |
| 安全標頭 | helmet middleware |
| SQL 注入防護 | Sequelize 參數化查詢 |
| XSS 防護 | 輸入驗證與轉義 |

---

## 14. 數據流程圖

### 14.1 交易訊號生成流程

```
用戶請求訊號
     │
     ▼
┌────────────────────────────────────────────────────────────────────────┐
│                        Backend API                                      │
│                                                                         │
│   1. 驗證請求參數 (pair, timeframe)                                     │
│   2. 查詢 Redis 快取                                                    │
│      ├─ 命中: 返回快取結果                                              │
│      └─ 未命中: 繼續處理                                                │
│                                                                         │
│   3. 獲取市場數據 (混合模式)                                            │
│      ├─ 從 PostgreSQL 讀取 99 根歷史 K 線                               │
│      └─ 從 Twelve Data API 獲取 1 根最新 K 線                           │
│                                                                         │
└─────────────────────────┬──────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                        ML Engine                                         │
│                                                                          │
│   4. 數據預處理                                                          │
│      ├─ 計算技術指標 (SMA, EMA, RSI, MACD, Bollinger)                   │
│      └─ 正規化數據 (MinMaxScaler)                                        │
│                                                                          │
│   5. LSTM 預測                                                           │
│      ├─ 輸入: 60 時間步 × 12 特徵                                        │
│      └─ 輸出: [Long, Short, Standby] 機率                                │
│                                                                          │
│   6. 情緒分析                                                            │
│      ├─ 抓取相關新聞 (NewsAPI / Google News RSS)                         │
│      ├─ FinBERT 分析新聞情緒                                             │
│      └─ VADER 分析央行政策                                               │
│                                                                          │
│   7. 訊號融合                                                            │
│      ├─ 計算時間框架權重                                                 │
│      ├─ technical_score × tech_weight + sentiment_score × sent_weight   │
│      └─ 判定訊號強度 (weak/moderate/strong/very_strong)                  │
│                                                                          │
└─────────────────────────┬───────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                        Backend API                                       │
│                                                                          │
│   8. 組裝回應                                                            │
│      {                                                                   │
│        "signal": "buy",                                                  │
│        "confidence": 0.75,                                               │
│        "signalStrength": "strong",                                       │
│        "marketCondition": "trending",                                    │
│        "entryPrice": 1.0850,                                             │
│        "stopLoss": 1.0820,                                               │
│        "takeProfit": 1.0900,                                             │
│        "indicators": { ... },                                            │
│        "sentiment": { ... }                                              │
│      }                                                                   │
│                                                                          │
│   9. 快取結果 (Redis, TTL: 60 秒)                                        │
│   10. 返回回應給用戶                                                     │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### 14.2 訂閱通知流程

```
用戶訂閱 EUR/USD
     │
     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│   Bot (Discord/LINE)                                                     │
│   1. 解析用戶指令                                                        │
│   2. 呼叫 Backend API: POST /api/v1/notifications/subscribe             │
└─────────────────────────┬───────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────────────┐
│   Backend API                                                            │
│   3. 驗證用戶存在 (若不存在則建立)                                       │
│   4. 建立訂閱記錄 (user_subscriptions 表)                                │
│   5. 返回成功回應                                                        │
└─────────────────────────┬───────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────────────┐
│   Bot (Discord/LINE)                                                     │
│   6. 發送確認訊息給用戶                                                  │
└─────────────────────────────────────────────────────────────────────────┘

... 時間經過 ...

┌─────────────────────────────────────────────────────────────────────────┐
│   Signal Monitoring Service (每 15 分鐘)                                 │
│   7. 獲取所有貨幣對最新訊號                                              │
│   8. 比對 Redis 中的舊訊號                                               │
│   9. 偵測到 EUR/USD 信號變化: HOLD → BUY                                 │
│   10. 查詢訂閱 EUR/USD 的用戶                                            │
│   11. 發布 Redis Pub/Sub 事件到 'signal-change' 頻道                     │
└─────────────────────────┬───────────────────────────────────────────────┘
                          │
          ┌───────────────┴───────────────┐
          ▼                               ▼
┌─────────────────────────┐   ┌─────────────────────────────────────────┐
│   Discord Bot           │   │   LINE Bot                               │
│   12. 接收事件          │   │   12. 接收事件                           │
│   13. 過濾 discord 訂閱  │   │   13. 過濾 line 訂閱                     │
│   14. 發送 Embed 通知    │   │   14. 發送 Push Message 通知             │
└─────────────────────────┘   └─────────────────────────────────────────┘
```

---

## 15. 部署指南

### 15.1 前置需求

| 軟體 | 最低版本 | 說明 |
|------|----------|------|
| Node.js | 18.0.0 | JavaScript 執行環境 |
| Python | 3.9 | ML Engine |
| PostgreSQL | 14.0 | 主資料庫 |
| Redis | 6.0 | 快取/Pub/Sub |
| PM2 | latest | Node.js 程序管理 |

### 15.2 安裝步驟

```bash
# 1. 克隆專案
git clone https://github.com/LazOof69/AIFX_v2.git
cd AIFX_v2

# 2. 安裝 Backend 依賴
cd backend && npm install

# 3. 安裝 ML Engine 依賴
cd ../ml_engine
python -m venv venv
source venv/bin/activate
pip install -r requirements.txt

# 4. 安裝 Discord Bot 依賴
cd ../discord_bot && npm install

# 5. 安裝 LINE Bot 依賴
cd ../line_bot && npm install

# 6. 設置環境變數
cd ..
cp backend/.env.example backend/.env
# 編輯 backend/.env 配置必要的環境變數

# 7. 資料庫遷移
cd backend
npx sequelize-cli db:migrate

# 8. 啟動所有服務
cd ..
./start-all-services.sh
```

### 15.3 PM2 配置範例

```javascript
// ecosystem.config.js
module.exports = {
  apps: [
    {
      name: 'aifx-backend',
      script: 'backend/src/server.js',
      instances: 1,
      autorestart: true,
      watch: false,
      max_memory_restart: '1G',
      env: {
        NODE_ENV: 'production',
        PORT: 3000
      }
    },
    {
      name: 'aifx-discord-bot',
      script: 'discord_bot/bot.js',
      instances: 1,
      autorestart: true,
      watch: false
    },
    {
      name: 'aifx-line-bot',
      script: 'line_bot/bot.js',
      instances: 1,
      autorestart: true,
      watch: false,
      env: {
        PORT: 3001
      }
    }
  ]
};
```

### 15.4 系統服務 (systemd)

```ini
# /etc/systemd/system/aifx-ml-engine.service
[Unit]
Description=AIFX ML Engine
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/root/AIFX_v2/ml_engine
ExecStart=/root/AIFX_v2/ml_engine/venv/bin/python api/ml_server.py
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
```

---

## 16. 環境變數完整列表

### 16.1 Backend (.env)

```env
# 服務配置
NODE_ENV=development
PORT=3000

# 資料庫
DATABASE_URL=postgresql://user:password@localhost:5432/aifx_db

# Redis
REDIS_URL=redis://localhost:6379
REDIS_DB=2

# JWT 認證
JWT_SECRET=your-jwt-secret-key-min-32-chars
JWT_REFRESH_SECRET=your-jwt-refresh-secret-key
JWT_EXPIRES_IN=15m
JWT_REFRESH_EXPIRES_IN=7d

# 外部 API
TWELVE_DATA_KEY=your-twelve-data-api-key

# ML Engine
ML_API_URL=http://localhost:8000

# Discord Bot
DISCORD_BOT_TOKEN=your-discord-bot-token
DISCORD_CLIENT_ID=your-discord-client-id
DISCORD_SIGNAL_CHANNEL_ID=your-signal-channel-id

# LINE Bot
LINE_CHANNEL_ACCESS_TOKEN=your-line-channel-access-token
LINE_CHANNEL_SECRET=your-line-channel-secret

# 服務間認證 (64 字元十六進位)
API_KEY=your-discord-api-key-64-hex-chars
LINE_BOT_API_KEY=your-line-api-key-64-hex-chars

# Admin 管理後台
ADMIN_USERNAME=admin
ADMIN_PASSWORD=your-admin-password
ADMIN_JWT_SECRET=your-admin-jwt-secret

# 日誌
LOG_LEVEL=info
LOG_FILE=/root/AIFX_v2/logs/backend.log
```

### 16.2 ML Engine (.env)

```env
# ML Engine
ML_ENGINE_PORT=8000

# Twelve Data API
TWELVE_DATA_KEY=your-api-key

# Backend API (用於數據存取)
BACKEND_API_URL=http://localhost:3000
ML_ENGINE_API_KEY=your-api-key

# 情緒分析
NEWS_API_KEY=your-newsapi-key

# 模型配置
MODEL_PATH=./saved_models
SENTIMENT_MODEL=ProsusAI/finbert
```

### 16.3 Discord Bot (.env)

```env
DISCORD_BOT_TOKEN=your-bot-token
DISCORD_CLIENT_ID=your-client-id
DISCORD_SIGNAL_CHANNEL_ID=your-channel-id
BACKEND_API_URL=http://localhost:3000
API_KEY=your-64-char-api-key
REDIS_URL=redis://localhost:6379
REDIS_DB=2
```

### 16.4 LINE Bot (.env)

```env
LINE_CHANNEL_ACCESS_TOKEN=your-channel-token
LINE_CHANNEL_SECRET=your-channel-secret
BACKEND_API_URL=http://localhost:3000
LINE_BOT_API_KEY=your-64-char-api-key
REDIS_URL=redis://localhost:6379
REDIS_DB=2
PORT=3001
```

---

## 17. 開發歷程

### 17.1 主要里程碑

| 日期 | 功能 | 說明 |
|------|------|------|
| 2025-11-26 | 交易週期功能 | 實作日內/周內/月內/季內四種週期 |
| 2025-11-27 | 訂閱通知 MVP | Redis Pub/Sub + Discord 通知 |
| 2025-11-28 | 訂閱通知增強 | Embed 格式 + 冷卻機制 |
| 2025-11-28 | LINE Bot 整合 | 完整 LINE Bot 功能 |
| 2025-11-29 | 數據收集系統 | 混合模式 + Cron 排程 |
| 2025-12-02 | 文件整合 | 整合所有開發文件 |

### 17.2 重要 Bug 修復

| 問題 | 檔案 | 修復方式 |
|------|------|----------|
| 快取雙重解析 | forexService.js:53 | 移除多餘的 JSON.parse() |
| Discord 互動超時 | discord_bot/bot.js | 實作 deferWithRetry() 重試機制 |
| LINE 用戶註冊失敗 | usersController.js | 修正 API Key 認證邏輯 |

---

## 18. 常見問題

### 18.1 連線問題

**Q: Backend 連不上資料庫？**
```bash
# 檢查 PostgreSQL 服務
sudo systemctl status postgresql

# 檢查連線字串格式
DATABASE_URL=postgresql://user:password@localhost:5432/aifx_db
```

**Q: Redis 連線失敗？**
```bash
# 檢查 Redis 服務
redis-cli ping
# 應該回應 PONG
```

**Q: ML Engine 連線失敗？**
```bash
# 檢查 ML Engine 是否運行
curl http://localhost:8000/health
```

### 18.2 Bot 問題

**Q: Discord Bot 指令沒有出現？**
```bash
# 重新部署斜線命令
cd discord_bot
node deploy-commands.js
```

**Q: LINE Bot 收不到訊息？**
1. 確認 Webhook URL 設定正確
2. 確認 Channel Access Token 有效
3. 檢查 PORT=3001 是否被佔用

**Q: 訂閱後收不到通知？**
1. 確認 Redis 服務運行中
2. 確認 Backend 信號監控服務運行中
3. 檢查訂閱是否成功建立

### 18.3 ML Engine 問題

**Q: 模型載入失敗？**
```bash
# 確認模型檔案存在
ls ml_engine/saved_models/
```

**Q: 情緒分析回應慢？**
- 首次載入 FinBERT 模型需要下載約 400MB
- 建議預先啟動服務讓模型載入完成

**Q: API 配額不足？**
- 啟用混合模式從資料庫讀取歷史數據
- 檢查 Cron 排程是否正確

---

## 免責聲明

1. 本系統提供的交易訊號**僅供參考**，不構成投資建議
2. 外匯交易涉及高風險，可能導致資金損失
3. 過去表現不保證未來結果
4. 用戶應自行評估風險並做出交易決策

---

## 授權

MIT License

---

<div align="center">

**AIFX v2** - *Empowering Traders with AI*

[GitHub](https://github.com/LazOof69/AIFX_v2) • [回報問題](https://github.com/LazOof69/AIFX_v2/issues)

**最後更新**: 2025-12-03

</div>
