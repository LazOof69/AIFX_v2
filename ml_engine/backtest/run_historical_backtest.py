...
Executes backtests using historical market data and ML-generated signals
"""
...
def generate_html_report(metrics_list: list, chart_files: list, output_file: str = '/tmp/historical_backtest_report.html'):
    """
...
<head>
    <meta charset="UTF-8">
    <title>歷史回測報告 - Historical Backtest Report (ML-Generated Signals)</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <h1>📊 AIFX v2 歷史回測報告 (Historical Backtest Report) <span class="badge ml">ML-Generated Signals</span></h1>
        <p><strong>生成時間:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
        <p><strong>回測策略:</strong> 信號反轉出場 (Signal Reversal Exit)</p>
        <p><strong>信號來源:</strong> ML模型預測 + 技術分析後備</p>
        <p><strong>回測對象:</strong> {', '.join(PAIRS)} × {len(PERIODS)} 個交易週期</p>
        <p><strong>數據期間:</strong> 最近90天歷史市場數據</p>
...
"""
    ...
    html += f"""
        ...
        <p>此回測使用ML模型生成交易信號。當ML API不可用時，會自動切換到簡單技術分析後備策略（SMA交叉）。</p>
        <ul>
            <li><strong>信號生成方式:</strong> ML預測 (主要) + 技術分析 (後備)</li>
            <li><strong>歷史數據窗口:</strong> 最近100根K線</li>
            <li><strong>出場策略:</strong> 信號反轉（long→short, short→standby）</li>
        </ul>
        ...
        <div class="footer">
            <p>🤖 Generated by AIFX v2 Historical Backtest Engine (ML-Powered)</p>
...
def main():
    ...
    print("=" * 80)
    print("🚀 AIFX v2 Historical Backtest Engine (ML-Powered)")
    print("=" * 80)
    print(f"\n📋 Configuration:")
    print(f"   Pairs: {', '.join(PAIRS)}")
    print(f"   Periods: {', '.join(PERIODS.keys())}")
    print(f"   Strategy: Signal Reversal Exit")
    print(f"   Signal Source: ML Prediction + Technical Analysis Fallback")
    print(f"   Initial Balance: $10,000")
    print(f"   Risk per Trade: 2%")
    print(f"   Historical Data: Last 90 days")
    print("\n" + "=" * 80)
    ...
    for pair in PAIRS:
        for period, timeframe in PERIODS.items():
            print(f"\n🔄 Running historical backtest: {pair} - {period} ({timeframe})")

            backtest = HistoricalBacktest(pair, timeframe, period)
            backtest.connect_db()
            ...
            try:
                ...
                if metrics:
                    ...
                    backtest_id = backtest.save_results(metrics)
                    ...
                    print(f"⚠️  Skipped: No market data available")
    ...
    if all_metrics:
        ...
        print(f"   Average Max Drawdown: {df['max_drawdown_pct'].mean():.2f}%")
        print("\n" + "=" * 80)
    else:
